...
from .views import login, launch, upload, consult, get_jwks, configure, score, scoreboard
...
urlpatterns = [
    ...
    url(r'^configure/(?P<launch_id>[\w-]+)/(?P<difficulty>[\w-]+)/$', configure, name='app-configure'),
    url(r'^api/score/(?P<launch_id>[\w-]+)/(?P<earned_score>[\w-]+)/(?P<time_spent>[\w-]+)/$', score,
        name='app-api-score'),
    url(r'^api/scoreboard/(?P<launch_id>[\w-]+)/$', scoreboard, name='app-api-scoreboard'),
]
